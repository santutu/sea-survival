...
namespace Santutu.Core.GameObjectTraveler.Runtime
{
    public static partial class GameObjectExtensions
    {
        ...
        public static IEnumerable<GameObject> SelfAncestors(this IEnumerable<GameObject> source)
        {
            foreach (var item in source)
            {
                var e = item.SelfAncestors().GetEnumerator();
                while (e.MoveNext())
                {
                    yield return e.Current;
                }
        ...
        public static IEnumerable<GameObject> Below(this IEnumerable<GameObject> source, Func<Transform, bool> descendIntoChildren = null)
        {
            foreach (var item in source)
            {
                var e = item.Below(descendIntoChildren).GetEnumerator();
                while (e.MoveNext())
                {
                    yield return e.Current;
                }
        ...
        public static IEnumerable<GameObject> SelfBelow(this IEnumerable<GameObject> source, Func<Transform, bool> descendIntoChildren = null)
        {
            foreach (var item in source)
            {
                var e = item.SelfBelow(descendIntoChildren).GetEnumerator();
                while (e.MoveNext())
                {
                    yield return e.Current;
                }
