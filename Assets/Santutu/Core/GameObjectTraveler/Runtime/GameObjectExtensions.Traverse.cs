...
namespace Santutu.Core.GameObjectTraveler.Runtime
{
    ...
    public static partial class GameObjectExtensions
    {
        ...
        public static IEnumerable<GameObject> SelfAbove(this GameObject origin, GameObject until = null)
        {
            yield return origin;

            if (until != null && origin.transform.parent && origin.transform.parent.gameObject == until)
            {
                yield return until;
            }

            if (origin.transform.parent)
            {
                foreach (var go in SelfSiblingAbove(origin.transform.parent.gameObject, until))
                {
                    yield return go;
                }
            }
        ...
        public static IEnumerable<GameObject> SelfSiblingAbove(this GameObject origin, GameObject until)
        {
            foreach (var go in origin.SelfSibling())
            {
                yield return go;
            }

            if (origin.transform.parent && origin.transform.parent.gameObject == until)
            {
                yield return until;
            }
        ...
        public static AncestorsEnumerable SelfAncestors(this GameObject origin)
        {
            return new AncestorsEnumerable(origin, true);
        }

        /// <summary>Returns a collection of the descendant GameObjects.</summary>
        public static DescendantsEnumerable Below(this GameObject origin, Func<Transform, bool> descendIntoChildren = null)
        {
        ...
        public static DescendantsEnumerable SelfBelow(this GameObject origin, Func<Transform, bool> descendIntoChildren = null)
        {
            return new DescendantsEnumerable(origin, true, descendIntoChildren);
        }

        public static IEnumerable<GameObject> Sibling(this GameObject origin)
        {
            foreach (var gameObject in origin.BeforeSelf())
            {
                yield return gameObject;
            }

            foreach (var gameObject in origin.AfterSelf())
            {
                yield return gameObject;
            }
        }

        public static IEnumerable<GameObject> SelfSibling(this GameObject origin)
        {
            yield return origin;
            foreach (var gameObject in origin.Sibling())
            {
                yield return gameObject;
            }
        }
